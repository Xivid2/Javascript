var NodeFactory = {
	...
	createTabs: function( numberTabs ) {
		...
		for ( var i = 0; i < numberTabs; i ++ ) {
			...
			var newTab = NodeFactory.addNewButton( tabConfig );
			tabDiv.appendChild( newTab );

			var contentSmallConfig = {
				classNames: ["hiddenContent", "smallContent"],
				attributes: {
					id: "content" + "Task" + ( i + 2 )
				}
			};
			var newSmallContent = NodeFactory.addNewDiv( contentSmallConfig );
			newContent.appendChild( newSmallContent );
		}
